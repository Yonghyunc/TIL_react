...
contract SaleAnimalToken {
    ...
    function setForSaleAnimalToken(uint256 _animalTokenId, uint256 _price) public {
        address aniamlTokenOwner = mintAnimalTokenAddress.ownerOf(_animalTokenId);
    
        require(aniamlTokenOwner == msg.sender, "Caller is not animal token owner.");
        require(_price > 0, "Price is zero or lower.");
        require(animalTokenPrices[_animalTokenId] == 0, "This animal token is already on sale.");
        require(mintAnimalTokenAddress.isApprovedForAll(aniamlTokenOwner, address(this)), "Animal token owner did not approve token.");
